...
const FREE_STYLE = 'flex-1 min-w-0 rounded-[9px] border border-dashed border-line bg-panel px-[13px] py-[9px] text-[13.5px] text-muted';
const OTHER_STYLE =
    ...
    <div className="flex flex-col gap-1.5">
      {rows.map((row) => (
        <div key={row.key} className="flex items-stretch gap-3">
          <span className="text-faint w-24 flex-none pt-2.5 text-xs font-medium [font-variant-numeric:tabular-nums]">
            {row.timeLabel}
          </span>
